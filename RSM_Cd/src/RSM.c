...
int main(int argc, char *argv[]) {
  ...
  double Cd_ads = 0.0;    /*ballistic co-efficient for adsorbed surface*/
  double Cd_srf = 0.0;    /*ballistic co-efficient for clean surface*/
  double fsc = 0.0;       /*fraction of O-covered surface*/ 
...
double AreaLookUp(struct area_struct *AreaData, struct initCd_struct *initCd) {
  ...
  double MinPitch = -4.99*M_PI/180.0;
  double MaxPitch = 4.99*M_PI/180.0;
  double MinYaw = -9.96*M_PI/180.0; /* Note by symmetry, this extends to -5.0 */
  double MaxYaw = 9.96*M_PI/180.0;
...
void read_input_file(struct initCd_struct *initCd) {
  ...
  if(initCd->pitch < -5.0 || initCd->pitch > 5.0) {
  ...
  if(initCd->yaw < -10.0 || initCd->yaw > 10.0) {
