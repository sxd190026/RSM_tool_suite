...
def run_tpmc(NPROCS, speciesnames, ispec, rtype, outdir):
    ...
    outname = "GRACE_"+speciesnames[ispec]+"_"+rtype+"_set.dat"
...
def tpmc_loop(NPROCS, NENS, mcmcdir, tpmdir):
    ...
    choose_species =[0];
    choose_mode = 0 #0 = Training, 1 = Test
    
    os.chdir(tpmdir)

    #for idx in range(2):
    idx = choose_mode;
    for ispec in choose_species:
...
if __name__ == '__main__':
    ...
    ####################### INPUTS ##########################
    NENS = 2000          #NUMBER OF ENSEMBLE MEMBERS
    NPROCS = 4          #NUMBER OF PROCESSORS FOR SIMULATION
    RSMNAME = "GRACE"      #NAME OF OUTPUT RSM FILE
